...
def cmd_sim(argv: list[str]) -> int:
    ...
    while True:
        ...
        if cmd in ("quit", "exit", "q"):
            ...
            # Show available pool summary (normal + desecrated + essence + runes)
            affix = parts[1] if len(parts) > 1 else ""
            min_lv_str = _parse_flag(parts, "--min")
            min_lv = int(min_lv_str) if min_lv_str else 0
            filter_str = _parse_flag(parts, "--filter")
            pool_filter: set[str] = set()
            if filter_str:
                pool_filter = {f.strip().lower() for f in filter_str.split(",")}
            ...
            # Normal pool
            if not pool_filter or "normal" in pool_filter:
            ...
            if not pool_filter or "desecrated" in pool_filter:
            ...
            # Essence pool
            if not pool_filter or "essence" in pool_filter:
            ...
            for _rpool, _rname in _RPN.items():
                if pool_filter and _rpool not in pool_filter and "runes" not in pool_filter:
                    continue
